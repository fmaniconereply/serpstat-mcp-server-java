...
public abstract class BaseToolHandler {
    ...
    protected BaseToolHandler(SerpstatApiClient apiClient) {
        if (apiClient == null) {
            throw new NullPointerException("apiClient must not be null");
        }
    ...
    /**
     * Handles the execution of a tool call, processes the response, and logs the
     * results.
     *
     * @param exchange   The `McpSyncServerExchange` instance used for logging
     *                   notifications.
     * @param arguments  A map of arguments required for the tool execution.
     * @param methodName The name of the method being executed, used for logging
     *                   purposes.
     * @param executor   A functional interface that defines the execution logic for
     *                   the tool.
     * @return A `CallToolResult` containing the processed result or an error
     *         message, along with a flag
     ...
     *         <li>Handles and logs any exceptions that occur during execution,
     *         including validation errors,
     *         API errors, and unexpected errors.</li>
     *         </ul>
     *
     *         <p>
     *         Exceptions are caught and logged, and appropriate error messages are
