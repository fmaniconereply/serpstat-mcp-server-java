 ...
 * 
    * These tests validate the basic structure and validation logic of the DomainCompetitor model.
    * They ensure that the model can handle typical competitor data, validate domain formats, and check relevance scoring.
 */
...
class DomainCompetitorTest {
    ...
    private String assessThreatLevel(double visibilityGap, double trafficRatio, double overlapPercentage) {
        if (visibilityGap > 30 && trafficRatio > 2.5 && overlapPercentage > 30) return "high";
        if (visibilityGap > 15 && trafficRatio > 1.5 && overlapPercentage > 20) return "medium";
        return "low";
    }
    ...
    private String calculateOverallTrend(double visibilityChange, double trafficChange, int keywordChange) {
        ...
        if (visibilityChange > 0) positiveIndicators++; else negativeIndicators++;
        if (trafficChange > 0) positiveIndicators++; else negativeIndicators++;
        if (keywordChange > 0) positiveIndicators++; else negativeIndicators++;
        
        if (positiveIndicators > negativeIndicators) return "growing";
    ...
    void testCompetitiveAnalysisIntegration() {
        ...
        Map<String, Object> temporalData = new HashMap<>();        temporalData.put("visibility_change_30d", 2.5); // Gained 2.5% visibility (positive)
        ...
        assertTrue((Double) competitiveIntelligence.get("opportunity_score") > 0, "Opportunity score should be positive");
